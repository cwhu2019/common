...
public class CRC8Util {
	 ...
	 * 描述表
	 */
	private final static char dscrc_table[] = { 0, 94, 188, 226, 97, 63, 221, 131, 194, 156, 126, 32, 163, 253, 31, 65,
	 ...
	 * @param datas
	 * @param start  起始校验位置
	 * @param len    从start开始指定的长度
	 * @return
	 */
	public static byte getCRC8(byte srcCrc, byte[] datas, int start, int len) {
		byte crc = srcCrc;
		if (datas != null) {
			for (int i = start; i < len + start; i++) {
				crc = (byte) dscrc_table[0x00ff & (crc ^ datas[i])];
			}
		}
		return crc;
	}
	 ...
	 * @return
	 */
	public static byte getCRC8(byte srcCrc, byte[] datas) {
		byte crc = srcCrc;
		if (datas != null) {
			crc = getCRC8(srcCrc, datas, 0, datas.length);
		}
		return crc;
	}
	 ...
	 * @return
	 */
	public static byte getCRC8(byte[] datas) {
		return getCRC8((byte) 0, datas);
	}
}
